...
const ConsultaDeReclamo = () => {
  const [documento, setID] = useState(""); // Estado para almacenar el ID ingresado
  const [reclamos, setReclamos] = useState<Reclamo[]>([]); // Estado para almacenar los servicios recibidos

  useEffect(() => {
    const fetchServicios = async () => {
      try {
        const response = await fetch('http://192.168.1.8:5000/reclamos/getAll');
        if (!response.ok) {
          throw new Error('Error al obtener los servicios');
        }
        const data = await response.json();
        setReclamos(data); // Almacenar los servicios recibidos en el estado local
  ...
  const reclamosFiltrados = reclamos.filter(reclamo => {
    if (documento.trim() === "") {
      ...
      return reclamo.documento === documento.trim();
    }
  });

  return (
    <View style={[styles.bsquedaDeServicio, styles.cardLayout]}>
      <Text style={styles.buscarServicios}>Buscar Reclamos</Text>
      ...
      <View style={styles.inputsParent}>
        ...
        <ScrollView>
          {reclamosFiltrados.map(reclamos => (
            <View key={reclamos.idReclamo} style={styles.servicioContainer}>
              <Text style={{ fontWeight: 'bold', marginBottom: 5, fontSize:16 }}>ID: {reclamos.idReclamo}</Text>
